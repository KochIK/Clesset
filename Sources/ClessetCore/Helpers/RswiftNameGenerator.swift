...
//  R.swift is created by Mathijs Kadijk and Tom Lokhorst.
//
 ...
 */
public struct RSwiftNameGenerator: Hashable, Comparable, Sendable {
    ...
    public init(name: String, lowercaseStartingCharacters: Bool = true) {
        ...
        let cleanedSwiftName = nameComponents.reduce(firstComponent) { $0 + $1.upperCaseFirstLetter() }
        ...
        let capitalizedSwiftName = lowercaseStartingCharacters ? RSwiftNameGenerator.lowercasePrefix(sanitizedSwiftName) : sanitizedSwiftName
    ...
    static func +(lhs: RSwiftNameGenerator, rhs: RSwiftNameGenerator) -> RSwiftNameGenerator {
        return RSwiftNameGenerator(rawValue: "\(lhs.value).\(rhs.value)")
    }
    
    public static func < (lhs: RSwiftNameGenerator, rhs: RSwiftNameGenerator) -> Bool {
        lhs.value < rhs.value
    }
}
...
struct SwiftNameGroups<T> {
    let uniques: [T]
    let duplicates: [(RSwiftNameGenerator, [String])] // Identifiers that result in duplicate Swift names
...
extension Sequence {
    func grouped(bySwiftIdentifier identifierSelector: @escaping (Iterator.Element) -> String) -> SwiftNameGroups<Iterator.Element> {
        var groupedBy = Dictionary(grouping: self, by: { RSwiftNameGenerator(name: identifierSelector($0)) })
        let empty = RSwiftNameGenerator(name: "")
