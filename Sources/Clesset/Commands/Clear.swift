...
// literally copy of `Analyze`...
// I'll refactor this a bit later
struct Clear: AsyncParsableCommand {
    
    static let cfonfiguration = CommandConfiguration(
        abstract: "Clear image assets in the project and lists unused `.imageset` files based on selected search strategies."
    )
    ...
    func run() async throws {
        try await Worker.analyze(
            ...
            shouldRemoveUnusedResources: true
        )
    }
